...
async function validateForm(event) {
  ...
  const phoneNumberPattern = /^\+\d{1,3}(-\d{1,2})?(-\d{3,4}){2}$/; // regex паттерн
  if (!phoneNumberPattern.test(phoneNumber.value.trim())) {
      alert('Please enter a valid phone number in the format +XXX-XX-XXXX-XXXX.');
      return false;
  }
...
async function fetchTables(date, time) {
  try {
    const response = await fetch(`http://localhost:8080/table/getBooking?date=${encodeURIComponent(date)}&time=${encodeURIComponent(time)}`);
...
function showTables(tables) {
  ...
  tables.forEach(table => {
      ...
      tableDiv.classList.add(table.booking_status); // Добавляем класс в зависимости от статуса
      ...
      console.log(`Added table: ${table.id}, status: ${table.booking_status}`); // Лог добавления столика
  });
...
(() => {
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    bookingsButton.addEventListener('click', async () => {
      bookingsModal.style.display = 'block';
      try {
        const response = await axios.get('http://localhost:8080/booking/get', { withCredentials: true });
        ...
        document.querySelectorAll('.delete-booking').forEach(button => {
          button.addEventListener('click', async () => {
            ...
            await axios.delete(`http://localhost:8080/booking/delete/${id}`, { withCredentials: true });
    ...
    categoriesButton.addEventListener('click', async () => {
      categoriesModal.style.display = 'block';
      try {
        const response = await axios.get('http://localhost:8080/category/get', { withCredentials: true });
        ...
        document.querySelectorAll('.delete-category').forEach(button => {
          button.addEventListener('click', async () => {
            const id = button.dataset.id;
            await axios.delete(`http://localhost:8080/category/delete/${id}`, { withCredentials: true });
            button.parentElement.parentElement.remove();
          });
        });
      } catch (error) {
    ...
    foodsButton.addEventListener('click', async () => {
      foodsModal.style.display = 'block';
      try {
        const response = await axios.get('http://localhost:8080/food/get', { withCredentials: true });
        const foods = response.data;
        const categoriesResponse = await axios.get('http://localhost:8080/category/get', { withCredentials: true });
        ...
        foods.forEach(food => {
          ...
          row.innerHTML = `
            ...
            <td>${(food.cost / 100).toFixed(2)} р.</td>
        ...
        document.querySelectorAll('.delete-food').forEach(button => {
          button.addEventListener('click', async () => {
            const id = button.dataset.id;
            await axios.delete(`http://localhost:8080/food/delete/${id}`, { withCredentials: true });
            button.parentElement.parentElement.remove();
          });
        });
      } catch (error) {
    ...
    document.getElementById('create-food-form').addEventListener('submit', async (event) => {
      ...
      try {
        await axios.post('http://localhost:8080/food/create', 
    ...
    async function fetchMenuItems() {
      try {
        const response = await fetch('http://localhost:8080/food/get');
